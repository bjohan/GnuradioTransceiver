...
class PlotWidget(Widget):
    def __init__(self):
        self.q = queue.Queue()
        self.xdata = np.array(list(range(10)))
        self.ydata = np.array(list(range(10)))
    ...
    def on_update_plot(self, *args):
        if not self.q.empty():
            ...
            self.xdata = np.array(list(range(len(data))))
    ...
    def buildGraticule(self, hlines, vlines):
        ...
        indices.extend(list(range(len(vt)/4)))
    ...
    def build_mesh(self):
        with self.lock:
            ...
            indices = list(range(len(self.xdata)))
            vertices = v2.ravel()
